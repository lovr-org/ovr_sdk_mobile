...
struct ovrPairT {
    ovrPairT() : First(), Second() {}

    ovrPairT(T1 first, T2 second) : First(first), Second(second) {}
...
class ovrJointMod {
   public:
    ...
    ovrJointMod() : Type(MOD_INVALID), JointIndex(-1) {}

    ovrJointMod(const ovrJointModType type, const int jointIndex, const OVR::Posef& pose)
        : Type(type), JointIndex(jointIndex), Pose(pose) {}
...
class ovrSkeleton {
   public:
    ...
    static void
    Transform(const OVR::Posef& transformPose, const OVR::Posef& inPose, OVR::Posef& outPose);

    static void TransformByParent(
        const OVR::Posef& parentPose,
        const int jointIndex,
        const OVR::Posef& inPose,
